...
class InterviewListAdapter(private val mContext: Context, private val interviews: MutableList<Interview>, private val listener: InterviewListAdapterListener) : RecyclerView.Adapter<InterviewListAdapter.ListHolder>() {
    val selectedItems: SparseBooleanArray

    private val registrationTable: RegistrationTable
    ...
    init {
        ...
        this.registrationTable = RegistrationTable(mContext)
    }
    ...
    override fun onBindViewHolder(holder: ListHolder, position: Int) {
        ...
        registration = registrationTable.getRegistrationById(interview.applicant)
        holder.from.text = registration!!.name
        holder.subject.text = "Village : " + registration!!.village
        holder.message.text = registration!!.subcounty
        holder.timestamp.text = "Total " + interview.getTotal()!!.toString()

        // displaying the first letter of From in icon text
        holder.iconText.text = registration!!.name.substring(0, 1)
    ...
    private fun applyProfilePicture(holder: ListHolder, interview: Interview) {
        if (!TextUtils.isEmpty(interview.picture)) {
            Glide.with(mContext).load(interview.picture)
                    .thumbnail(0.5f)
                    .crossFade()
            ...
            holder.imgProfile.setColorFilter(interview.color)
    ...
    private fun applyImportant(holder: ListHolder, interview: Interview) {
    ...
    private fun applyReadStatus(holder: ListHolder, interview: Interview) {
        if (interview.isRead) {
    ...
    interface InterviewListAdapterListener {
