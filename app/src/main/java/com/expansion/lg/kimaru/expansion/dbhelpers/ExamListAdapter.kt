...
class ExamListAdapter(private val mContext: Context, private val exams: MutableList<Exam>, private val listener: ExamListAdapterListener) : RecyclerView.Adapter<ExamListAdapter.ListHolder>() {
    ...
    override fun onBindViewHolder(holder: ListHolder, position: Int) {
        ...
        holder.from.text = "English: " + exam.english!!
        holder.subject.text = "Maths: " + exam.math!!
        holder.message.text = "Self assessment: " + exam.personality!!
        val dateAdded = Date(exam.dateAdded!! * 1000)
        val simpleDateFormat = SimpleDateFormat("MM/dd/yyyy' ' HH:mm:ss:S")
        holder.timestamp.text = simpleDateFormat.format(dateAdded)

        // displaying the first letter of From in icon text
        holder.iconText.text = exam.math.toString().substring(0, 1)

        // change the row state to activated
        holder.itemView.isActivated = selectedItems.get(position, false)

        //change the fontstyle depending on message read status (change this to whether passed or not)
        applyReadStatus(holder, exam)

        // handle message star
        applyImportant(holder, exam)

        // handle icon animation
        applyIconAnimation(holder, position)

        // display profile image
        applyProfilePicture(holder, exam)
    ...
    private fun applyProfilePicture(holder: ListHolder, exam: Exam) {
        if (!TextUtils.isEmpty(exam.getPicture())) {
            Glide.with(mContext).load(exam.getPicture())
                    .thumbnail(0.5f)
                    .crossFade()
    ...
    private fun applyImportant(holder: ListHolder, exam: Exam) {
        if (exam.isImportant) {
    ...
    private fun applyReadStatus(holder: ListHolder, exam: Exam) {
        if (exam.isRead) {
    ...
    interface ExamListAdapterListener {
