...
class CommunityUnitListAdapter(private val mContext: Context, private val communityUnits: MutableList<CommunityUnit>, private val listener: CommunityUnitListAdapterListener) : RecyclerView.Adapter<CommunityUnitListAdapter.ListHolder>() {
    ...
    override fun onBindViewHolder(holder: ListHolder, position: Int) {
        ...
        holder.from.text = communityUnit.communityUnitName
        holder.subject.text = "Households " + communityUnit.noOfHouseholds.toString() + "\nHouseholds per CHV " +
                communityUnit.householdPerChv
        if (!communityUnit.linkFacilityId.equals("", ignoreCase = true)) {
            holder.message.text = communityUnit.linkFacilityId // show Households and per Chv
            val l = LinkFacilityTable(mContext).getLinkFacilityById(communityUnit.linkFacilityId)
            if (l != null) {
                holder.message.text = l.facilityName
            } else {
                holder.message.text = "Villages " + communityUnit.numberOfVillages.toString()
            }
        } else {
            holder.message.text = "Villages " + communityUnit.numberOfVillages.toString()
        }

        holder.timestamp.text = DisplayDate(communityUnit.dateAdded).dateOnly()

        // displaying the first letter of From in icon text
        holder.iconText.text = communityUnit.communityUnitName.substring(0, 1)

        // change the row state to activated
        holder.itemView.isActivated = selectedItems.get(position, false)

        //change the fontstyle depending on message read status (change this to whether passed or not)
        applyReadStatus(holder, communityUnit)

        // handle message star
        applyImportant(holder, communityUnit)

        // handle icon animation
        applyIconAnimation(holder, position)

        // display profile image
        applyProfilePicture(holder, communityUnit)
    ...
    private fun applyProfilePicture(holder: ListHolder, communityUnit: CommunityUnit) {
        if (!TextUtils.isEmpty(communityUnit.getPicture())) {
            Glide.with(mContext).load(communityUnit.getPicture())
                    .thumbnail(0.5f)
                    .crossFade()
            ...
            holder.imgProfile.setColorFilter(communityUnit.getColor())
    ...
    private fun applyImportant(holder: ListHolder, communityUnit: CommunityUnit) {
        if (communityUnit.isNgoDoingIccm) {
    ...
    private fun applyReadStatus(holder: ListHolder, communityUnit: CommunityUnit) {
        if (communityUnit.isChvsTrained) {
    ...
    interface CommunityUnitListAdapterListener {
