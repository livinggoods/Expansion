...
class MappingListAdapter(private val mContext: Context, private val mappings: MutableList<Mapping>, private val listener: MappingListAdapterListener) : RecyclerView.Adapter<MappingListAdapter.ListHolder>() {
    ...
    override fun onBindViewHolder(holder: ListHolder, position: Int) {
        ...
        val mappingName = mapping.mappingName
        ...
        holder.from.text = mappingName
        holder.subject.text = mapping.contactPerson
        holder.message.text = mapping.contactPersonPhone
        holder.timestamp.text = DisplayDate(mapping.dateAdded).dateAndTime()

        // displaying the first letter of From in icon text
        holder.iconText.text = mapping.mappingName.substring(0, 1)
    ...
    private fun applyProfilePicture(holder: ListHolder, mapping: Mapping) {
    ...
    fun getPItemId(position: Int): String {
        return mappings[position].id
    }

    private fun applyImportant(holder: ListHolder, mapping: Mapping) {
    ...
    private fun applyReadStatus(holder: ListHolder, mapping: Mapping) {
        if (mapping.isRead()) {
    ...
    interface MappingListAdapterListener {
